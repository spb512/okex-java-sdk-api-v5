...
import com.okx.open.api.service.sub.account.SubAccountApiService;
import com.okx.open.api.service.sub.account.impl.SubAccountApiServiceImpl;
 ...
 *
 */
public class SubAccountApiTests extends SubAccountApiBaseTest {
	...
	/**
	 * 母账户查询子账户的交易账户详细资产余额（适用于母账户） Query detailed balance info of Trading Account of
	 * a sub-account via the master account (applies to master accounts only) GET
	...
	/**
	 * 子账户转账记录（仅适用于母账户） View sub-account list(applies to master accounts only) GET
	 ...
	 * accounts only) POST /api/v5/users/subaccount/delete-apikey
	 */
	@Test
	 ...
	 * accounts only) POST /api/v5/users/subaccount/modify-apikey
	 */
	@Test
	 ...
	 * accounts only) POST /api/v5/users/subaccount/apikey
	 */
	@Test
	...
	/**
	 * 查看子账户列表（仅适用于母账户） View sub-account list(applies to master accounts only) GET
	...
	/**
	 * 母账户控制子账户与子账户之间划转（仅适用于母账户） Master accounts manage the transfers between
	 * sub-accounts(applies to master accounts only) POST
